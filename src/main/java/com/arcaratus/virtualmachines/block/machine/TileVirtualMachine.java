...
/**
 * Literal carbon copy of TileMachineBase in Thermal Expansion
 * Needed here to make accessing necessary fields less of a PITA
 */
public abstract class TileVirtualMachine extends TilePowered implements IAccelerable, ITickable
{
    ...
    protected int secondaryChance = SECONDARY_BASE;
    protected int reuseChance = 0;

    /* AUGMENTS */
    protected boolean augmentSecondaryNull;

    public TileVirtualMachine()
    {
    ...
    {
        return VirtualMachines.INSTANCE;
    }

    @Override
    protected String getModVersion()
    {
        return VirtualMachines.VERSION;
    }
    ...
    protected boolean installAugmentToSlot(int slot)
    {
        ...
        {
            energyConfig.setDefaultParams(energyConfig.maxPower + getBasePower(level), smallStorage);
            energyMod += 15;
            return true;
        }
    ...
    public TextureAtlasSprite getTexture(int side, int pass)
    {
        if (pass == 0)
        {
            ...
            return side != facing ? TETextures.MACHINE_SIDE : isActive ? VMTextures.MACHINE_ACTIVE[getType()] : VMTextures.MACHINE_FACE[getType()];
        }
        else if (side < 6)
        {
